...
def get_default_key():
    """
    Busca a chave padrão a partir do st.secrets (configurado no painel do
...
def scrape_website_details(website_url):
    ...
    efetivamente conseguiu acessar o site ("Scrapling", "Requests" ou ""
    se nenhum dos dois conseguiu abrir a página), pra dar rastreabilidade
    real de que o bypass de bloqueio funcionou (e não só que algum dado
    apareceu via busca de fallback depois).
    """
    email, social = "", ""
    metodo = ""
    ...
    if text:
        ...
        # Nem Scrapling nem requests conseguiram abrir a página
        # (provável bloqueio não contornado, site fora do ar, ou timeout)
        metodo = ""
